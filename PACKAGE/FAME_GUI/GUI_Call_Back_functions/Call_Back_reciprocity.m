function Call_Back_reciprocity(hobj,event,h)
    
    [Popt] = FAME_GUI_Info_Get(h)
    
    recip = str2num( get(hobj,'string') );
    
    switch Popt.material.material_type
        case {'isotropic' 'biisotropic'} % isotropic, biisotropic
            fool_check_isotropic(recip,h)
        case 'anisotropic' % anisotropic
            fool_check_anisotropic(recip,h)
    end
    
    FAME_GUI_Info_Set('reciprocity', get(hobj,'string'), h);
end
