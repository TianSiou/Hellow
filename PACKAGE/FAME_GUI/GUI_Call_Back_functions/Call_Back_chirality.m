function Call_Back_chirality(hobj,event,h)
    
    [Popt] = FAME_GUI_Info_Get(h)
    
    chira = str2num( get(hobj,'string') );
    
    switch Popt.material.material_type
        case {'isotropic' 'biisotropic'} % isotropic, biisotropic
            fool_check_isotropic(chira,h)
        case 'anisotropic' % anisotropic
            fool_check_anisotropic(chira,h)
    end
    
    FAME_GUI_Info_Set('chirality', get(hobj,'string'), h);
end
