function [ freq, Ele_field, cpu_time, LS_iter, LS_cpu_time ] = FAME_Fast_Algorithms_Simple_Anisotropic( grid_num, wave_vec, B, Lambdas, eigen_wanted)
...
%     Lambdas.D_ks         = conj(Lambdas.D_k);
    ...
    cpu_time_start = tic;
    [ev,ew] = eigs( @(x) FAME_Matrix_Vector_Production_Anisotropic_invAr_Simple( x, B, Nx, Ny, Nz, N, Lambdas.Sigma_r, Lambdas.Pi_Qr, Lambdas.Pi_Qrs, Lambdas.D_k, Lambdas.D_ks, LS_tol),...
...
%         Output_eigvec = [ B.invB_eps.d11.*vec_y(1:N) + B.invB_eps.d12.*vec_y(N+1:2*N) + B.invB_eps.d13.*vec_y(2*N+1:3*N);
%                           B.invB_eps.d21.*vec_y(1:N) + B.invB_eps.d22.*vec_y(N+1:2*N) + B.invB_eps.d23.*vec_y(2*N+1:3*N);
%                           B.invB_eps.d31.*vec_y(1:N) + B.invB_eps.d32.*vec_y(N+1:2*N) + B.invB_eps.d33.*vec_y(2*N+1:3*N) ];   
