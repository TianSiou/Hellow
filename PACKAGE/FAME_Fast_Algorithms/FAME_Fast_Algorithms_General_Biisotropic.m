function [ freq, Ele_field, Mag_field, cpu_time, LS_iter, LS_cpu_time ] = FAME_Fast_Algorithms_General_Biisotropic(grid_num, wave_vec, B, Lambdas, eigen_wanted )
...
%     Lambdas.inv_Sigma_r = 1./ Lambdas.Sigma_r;   
%     B                = 1i*[spdiags(B_zeta,0,3*N,3*N), spdiags(B_mu,0,3*N,3*N);
%                            spdiags(-B_eps,0,3*N,3*N), spdiags(-B_xi,0,3*N,3*N)];
    ...
    cpu_time_start = tic;
    [ev,ew] = eigs( @(vec_x) FAME_Matrix_Vector_Production_Biisotropic_General( vec_x, B, Nx, Ny, Nz, N,...
                         Lambdas.inv_Sigma_r, Lambdas.Pi_Qr, Lambdas.Pi_Pr, Lambdas.Pi_Qrs, Lambdas.Pi_Prs, Lambdas.D_kx, Lambdas.D_ky, Lambdas.D_kz, Lambdas.F_ky, Lambdas.F_kz, LS_tol),...
...
%% Start to restore the eigenvectors
    for column_idx = 1 : size(Input_eigvec_mat,2)
        ...
        vec_y_ele = FAME_Matrix_Vector_Production_Pr_General(vec_x_ele, Nx, Ny, Nz, N, Lambdas.Pi_Pr, Lambdas.Pi_Prs, Lambdas.D_kx, Lambdas.D_ky, Lambdas.D_kz, Lambdas.F_ky, Lambdas.F_kz, 'normal');
        vec_y_mag = FAME_Matrix_Vector_Production_Qr_General(vec_x_mag, Nx, Ny, Nz, N, Lambdas.Pi_Qr, Lambdas.Pi_Qrs, Lambdas.D_kx, Lambdas.D_ky, Lambdas.D_kz, Lambdas.F_ky, Lambdas.F_kz, 'normal');
