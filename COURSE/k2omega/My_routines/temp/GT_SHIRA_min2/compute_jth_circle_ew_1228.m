function [ ew_unit_c, ev_on_c, ew_inside_c_new, ev_inside_c_new, no_deuplicate_ew, ...
    ...
    compute_jth_circle_ew_1228( dim_n, mtx_NME, mtx_M, mtx_L, rad, rad_old, max_wanted_ew_org, ...
    radius_case, vidObj, weighting_choosing, ew_unit_c, ev_on_c, ...
    ew_inside_c, ev_inside_c, ref_ev_outside_c, ref_ew_inside_c, ref_ev_inside_c, plot_flag, ...
    no_target_ew_on_c, no_target_cmp_ew, no_target_real_ew, deflation_flag)
...
rat_max_angle      = 1.7;
no_redoing_GTSHIRA = 0;

no_conv_ew    = no_target_ew_on_c + 2 * no_target_cmp_ew + no_target_real_ew;

while ( jj <= max_jj && strcmp(shrink,'no') && no_conv_ew < dim_n ) %20
    skill_flag     = 0;
    no_re_estimate = 0;
    overlapping    = 'no';
    Move_shift     = 'no';
            
    while ( strcmp(overlapping,'no') && no_re_estimate <= max_no_re_estimate )
        re_estimate_check = 'yes';
        no_skew_J_Lan     = 1;
    
        while ( no_skew_J_Lan <= max_no_skew_J_Lan && strcmp(re_estimate_check,'yes') )
            ...
            idx_target_ew_T = find(abs(ew_T - shift(jj,1))> 1.0e-4*abs(shift(jj,1)));
            ij              = idx_target_ew_T(1);
            
            angle_shift  = angle(shift(jj,1));
            angle_1st_ew = angle(ew_T(ij,1));
            ...
            if ( (abs(ew_T(ij,1)) >= rad_old || (angle_1st_ew - angle_shift) > 50/180*pi ) && residual(ij) < 1.0e-2)
                cn_radius   = abs(ew_T(ij,1) + 1 / ew_T(ij,1) - shift(jj,1) - 1 / shift(jj,1));
                idx_pos_img = find( imag(ew_T) > 0 );
                thm_pt      = esstimating_am_in_conv_circle(cn_radius, shift(jj,1));
                ...
                radius_moving(no_moving,1) = abs(thm_pt); 
                no_moving                  = no_moving + 1;
...
%         eigenwanted = mm - 10;
        ...
        elseif ( mm ~= 0 )
            ...
            if (~isempty(idx))
                ...
                min_ang_skew_Cay_ew = min(angle(ew_skew_Cayley(idx,jj)));
                if ( jj == 1 )
                    max_ang_ew = 0;
                else
                    ...
                    % 
                        ...
                        if ( max_ang_ew >= ang_shift - 1.0e-3 && max_ang_ew >= pi - 3.0e-3 ) % spectial case
                            ...
                            if ( diff_angle >= ang_shift2ew * 0.2 )
                                ...
                                fprintf('new_theta = %11.4e, min_ang_skew_Cay_ew = %11.4e in compute_jth_circle_ew \n', new_theta, min_ang_skew_Cay_ew)
                            else
                                if ( jj == 1 )
...
%         if ( strcmp(flag_stop,'no') )
            ...
            if ( ~isempty(ew_new) ) % GTSHIRA converges or not
                ...
                idx_chk_err   = find(abs(lambda_in) > 1+1.0e-8);
                if ( ~isempty(idx_chk_err) )
                    lambda_out             = ew_new(2:2:leng_ew);
                    lambda_in(idx_chk_err) = lambda_out(idx_chk_err);
                ...
                if ( ~isempty(idx_unit)) 
                    if ( ~isempty(ew_unit_c) )
                        for i1 = 1:length(idx_unit)
                            if (min(abs(lambda_in(idx_unit(i1))-ew_unit_c(:,1))) < tol )
                                idx_deupli(idx_unit(i1),1) = 1; 
                            elseif (min(abs(lambda_in(idx_unit(i1))-ew_unit_c(:,2))) < tol )
                                idx_deupli(idx_unit(i1),1) = 1;
                            else
...
%                     end
                ...
                if ( leng_idx_rsdl < no_ew_new/2 ) %( rad >= 0.01 && leng_idx_rsdl < no_ew_new/2 )
                    flag_redo_GTSHIRA = 'yes';
                    if ( rad < 6.0e-4 )
                        if ( real(sigma) > 0 )
                            sigma             = rad * exp(1i * mod(angle(sigma), 2*pi) * 0.993);
                        else
                            tt = mod(angle(sigma), 2*pi) * 1.007;
                            if ( tt <= pi )
                                sigma         = rad * exp(1i * tt);
                            else
                                sigma         = rad * exp(1i * mod(angle(sigma), 2*pi) * 0.998);
                            end
                        end
                    end
                    
                    if ( ~isempty(find(idx_new_ui_ew == 1, 1)) )
                        no_unit_ew        = no_unit_ew - mm3;
                    end
                ...
                if ( leng_idx_rsdl == 0 )
                    ...
                    if ( angle(sigma) >= 176/180*pi )
                        ...
                        elseif ( angle(sigma) >= 178.2/180*pi )
                            shrink     = 'yes';
                            fprintf('shrink in compute_jth_circle_ew \n');
                        else
                            eigenwanted       = 40;
                            flag_redo_GTSHIRA = 'yes';
                            sigma             = rad*exp(1i*178.5/180*pi); %exp(1i*178.8541/180*pi)
                        end
                    end
...
%                     if ( no_redoing_GTSHIRA >= 2 && rad <= 6.0e-4 && angle(sigma) < 175/180*pi)
%                         eigenwanted2 = 5;
%                     else
%                         eigenwanted2 = eigenwanted;
%                     end
%                     no_redoing_GTSHIRA = no_redoing_GTSHIRA + 1;
                    
                    fprintf('\n Re_doing GTSHIRA with sigma = %11.4e +(%11.4e)*1i \n', real(sigma), imag(sigma));
                        
                    [ deflation_enforce, lambda_in, ew_new, ev_new, conv_radius(jj,1), rsdl, ...
                            no_deuplicate_ew(jj,:), flag_cong_GTSHIRA, ew_unit_c, ev_on_c, ew_inside_c_new, ...
                    ...
                    skill_flag = 0;
                    if ( flag_cong_GTSHIRA == 1 && angle(sigma) >= 178.2/180*pi && rad < 6.0e-4 )
                        shrink     = 'yes';
                    elseif ( flag_cong_GTSHIRA == 0 && angle(sigma) >= 178.2/180*pi && rad < 6.0e-4 )
                        skill_flag = 0;
                    end
                end
...
%
            ...
            %if ( flag_cong_GTSHIRA == 0 && mod(angle(sigma),2*pi) < 179/180*pi )
                ...
                if ( leng_idx_rsdl > 0 ) %( leng_idx_rsdl < no_ew_new/2 )
            ...
            elseif ( flag_cong_GTSHIRA == 0 && checking_angle >= 178/180*pi && ~isempty(ew_no_dupli) )
                [ ew_new_1, ev_new, rsdl, cong_flag, ew_unit_c, ev_on_c, ew_inside_c_new, ev_inside_c_new, ...
                    no_ew_in_c, next_ref_ev_outside_c, no_ref_ev, flag_conv, no_unit_ew ] = find_loss_real_ew( dim_n, ...
                    mtx_NME, lambda_in, prev_ew, sigma, conv_radius(jj,1), rad, rad_old, ew_new, ev_new, tolerance, ...
                    ...
                    if ( isempty(idx_ew) )
                        ...
                        elseif ( ~isempty(idx_neg_real_ew) ) 
                            ...
                            if ( (prev_no_ew_isempty >= 3) || (prev_no_ew_isempty >= 2 && mod(angle(shift(jj,1)),2*pi) > 130/180*pi) ...
                                ...
                                %[conv_region_angle angle(shift(jj,1)) abs(conv_region_angle - angle(shift(jj,1)))/2 15/180*pi ]
                            end
                            ...
                            if ( rad < 6.0e-4 )
                                [new_sigma] = choose_new_shift( shift(jj,1), lambda_in, ew_inside_c_new(1:no_ew_in_c,1), ...
                                    rad, rad_old, conv_radius(jj,1), max_wanted_ew_org, rat_max_angle, prev_no_ew_isempty );
                            end
                        ...
                        ang_cong_ew  = max(angle(new_left_ew));
                        
                        if ( ~isempty(idx_neg_real_ew) )
                            tt              = no_ew_in_c_org+1:no_ew_in_c; 
                            ang_neg_real_ew = min(mod(angle(ew_inside_c_new(tt(idx_neg_real_ew))),2*pi));
                        else
                            ang_neg_real_ew = 20*pi;
                        end
                         
                        
                        if ( ~isempty(idx_neg_real_ew) && ang_neg_real_ew - ang_cong_ew <= 4/180*pi)
                            increment = 'yes';
                            fprintf('check_shrinking_cond (a) in compute_jth_circle_ew \n');
...
%                     end
                ...
                if ( isempty(index2) || skill_flag == 1 )
                    ...
                    if ( skill_flag == 1 )
                        idxx5 = find(idx_deupli == 1 & imag(lambda_in) > -1.0e-10 & abs(lambda_in) < rad );
                        if ( ~isempty(idxx5) )
                            [~,ij]          = min(abs(lambda_in(idxx5)));
                            ew_inner(jj, 1) = lambda_in(idxx5(ij));
                        end
                    end
                    %fprintf('ew_inner = 0 (i) with skill_flag = %1.0f\n', skill_flag)
                elseif ( skill_flag == 0 ) 
...
% %                 end
            ...
            %idx_test          = find( ew_inside_c_new == 0 );
            leng_ew           = no_ew_in_c; %idx_test(1) - 1; 
            idx_unit_ew       = find(abs(abs(ew_inside_c_new(1:leng_ew,1))-1) < 1.0e-10);
            idx_cmp_ew        = find(abs(imag(ew_inside_c_new(1:leng_ew,1))) > 1.0e-9 & abs(abs(ew_inside_c_new(1:leng_ew,1))-1) >= 1.0e-10);
            idx_real_ew       = setdiff(1:leng_ew, [idx_cmp_ew; idx_unit_ew]);
...
if ( strcmp(shrink, 'yes') || no_conv_ew >= dim_n )
...
%             idx_2 = abs(ew_inside_c_new(1:no_ew_in_c,1)) <= rad_old;
    ...
    [ rad, max_wanted_ew, radius_case ] = determine_radius_circle( ew_inner, rad_old, no_pos_real_ew, ...
        ...
        ew_inside_c_new(1:no_ew_in_c,1), prediction_pt(1:jj-1,1), radius_moving );
...
no_deuplicate_ew = no_deuplicate_ew(1:jj-1,:);
