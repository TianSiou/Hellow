function [ mtx_deflate, flag ] = construct_deflated_mtx_pone( lambda, ev_lambda, ev_lambda_inv, mtx_NME, sigma, LU_W, deflate_tol )
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here

nu       = (lambda + 1)./(lambda - 1);
leng_nu  = length(nu);
idx_real = find(abs(imag(nu)) <= 1.0e-12);
idx_pimg = find(abs(real(nu)) <= 1.0e-13);
