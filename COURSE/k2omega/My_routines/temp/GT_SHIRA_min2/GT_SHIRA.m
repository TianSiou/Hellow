function [ev, ew, Hess_H, UpTr_R, inv_Z, outer_it] = GT_SHIRA(n, mtx_A, mtx_Q, solve_LS_PQEP, ... 
        xini, eigswanted, nstp, itmax, tolerance, shift, dynamical_change_eigswanted) 
%
...
%for j = 1:itmax
   ...
   if converged < eigswanted
       ...
       if (no_conv_ew == nstp || no_conv_ew >= eigswanted)
           conv_flag = 1;
           if (no_conv_ew == nstp )
               converged = nstp;
           else
               converged = eigswanted;
           end
       else
...
%                    eigswanted                  = min(eigswanted, nstp);
           ...
           Restart_method = 'Implicit'; %'Implicit'; %'Schur';
