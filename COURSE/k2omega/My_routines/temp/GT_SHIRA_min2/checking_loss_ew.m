function [ doing_GTSHIRA_again, shift ] = checking_loss_ew( lambda, prev_ew, ref_ew_inside_c, ...
...
if ( ~isempty(idx_angle) )
    ...
    left_angle     = 0.95 * max(left_ang_thm_pt, max(tmp_ang2));
...
idx2 = find( abs(ref_ew_inside_c) <= pre_rad & ref_angle >= right_angle & ref_angle <= left_angle);
